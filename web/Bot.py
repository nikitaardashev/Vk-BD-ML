...
class Bot:
    def __init__(self):
        ...
        self.new_cats = sorted(['физика', 'математика'])
        ...
        self.latest_id = self.database_session.query(Groups.group_id).order_by(
    ...
    def command_admin(self, from_id):
        ...
        if user_status:
            ...
            self.database_session.add(
                UserStatuses(user_id=from_id, status='admin'))
        self.database_session.commit()
        ...
        if user_status.status == 'admin':
            if '#' in payload['button']:
                ...
                if gr_id > self.latest_id:
                    ...
                    old_group = self.database_session.query(
                        GroupsIds).get(self.latest_id)
                    self.database_session.add(Groups(group_id=self.latest_id,
                                                     name=old_group.name,
                                                     subject=cat,
                                                     link=old_group.link))
            ...
            group = self.database_session.query(GroupsIds).order_by(
                GroupsIds.group_id.asc()
            ).filter(GroupsIds.group_id > self.latest_id).first()
